...
class Pawn < Piece #pawn is a piece
	...
	def move(x, y)
		...
		if((@is_team_red == true and ydist < 0) or (@is_team_red == false and ydist > 0))
			puts("cannot move pawn backwards");
			return false;
		end			
		
		if(ydist.abs > 1 or xdist.abs > 1) #checks if the distance the pawn has moved more than one space
